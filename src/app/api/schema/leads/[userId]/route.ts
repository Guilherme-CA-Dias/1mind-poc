...
export async function GET(
	...
	if (!schema) {
		// Use the default leads schema
		const defaultSchema = DEFAULT_SCHEMAS.leads;

		if (!defaultSchema) {
			console.warn("No default leads schema found");
	...
	try {
		...
		if (!schema) {
			...
			schema = await FieldSchema.create({
				...
				required: defaultSchema?.required || ["Id"],
			});
		}
		...
		console.error("Error adding field to leads schema:", error);
...
export async function DELETE(
	...
	if (coreFields.includes(fieldName)) {
		return NextResponse.json(
			{
				error: "Cannot delete core field from leads schema",
			},
			{ status: 400 }
		);
	}
